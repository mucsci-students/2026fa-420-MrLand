...
class TestFacultyGetters(unittest.TestCase):
    ...
    def test_get_name_rejects_duplicate_case_insensitive(self):
        ...
        faculty.faculty_members.append(existing)

        # First attempt reuses the existing name in a different case,
        # so get_name() should recurse and prompt again.
        inputs = ["dr. smith", "Dr. Jones"]
        with patch("builtins.input", side_effect=inputs):
            with patch("builtins.print") as mock_print:
                result = faculty.get_name()
    ...
    def test_faculty_times_multiple_ranges_single_day(self):
        ...
        self.assertEqual(times["MON"], ["09:00-12:00", "13:00-17:00"])
...
class TestViewFacultyNames(unittest.TestCase):
    ...
    def test_empty_list_returns_false(self):
        with patch("builtins.print") as mock_print:
            result = faculty.view_faculty_names()
    ...
    def test_prints_all_faculty_member_names(self):
        for name in ["Dr. Smith", "Dr. Jones", "Dr. Lee"]:
            faculty.faculty_members.append(faculty.FacultyConfig(
                name=name,
                maximum_credits=12,
                minimum_credits=6,
                unique_course_limit=3,
...
class TestViewFacultyEdgeCases(unittest.TestCase):
    ...
    def test_view_faculty_empty_list(self):
        with patch("builtins.print") as mock_print:
            faculty.view_faculty()
        mock_print.assert_called_once_with("No faculty members found.")
...
class TestModifyFacultyEdgeCases(unittest.TestCase):
    ...
    def setUp(self): 
        ...
        self.faculty_member = faculty.FacultyConfig( 
            ...
            lab_preferences={}, 
            mandatory_days={"MON"} ) 
        faculty.faculty_members.append(self.faculty_member)
    ...
    def test_modify_faculty_invalid_index_then_valid(self):
        ...
        inputs = ["not_a_number", "1", "2", "20"]
        with patch("builtins.input", side_effect=inputs):
            with patch("builtins.print"):
                faculty.modify_faculty()
        self.assertEqual(faculty.faculty_members[0].maximum_credits, 20)
    ...
    def test_modify_faculty_index_out_of_range(self):
        ...
        self.assertEqual(faculty.faculty_members[0].maximum_credits, 12)
    ...
    def test_modify_faculty_invalid_attribute_choice_leaves_data_unchanged(self):
        ...
        self.assertIn("Invalid selection.", printed_text)
        self.assertEqual(faculty.faculty_members[0].maximum_credits, 12)
        self.assertEqual(faculty.faculty_members[0].minimum_credits, 6)
    ...
    def test_modify_faculty_name(self):
        inputs = ["1", "1", "Dr. Renamed"]
        with patch("builtins.input", side_effect=inputs):
            with patch("builtins.print"):
                faculty.modify_faculty()
        self.assertEqual(faculty.faculty_members[0].name, "Dr. Renamed")

    def test_modify_faculty_mandatory_days(self):
        inputs = ["1", "10", "tue", "thu", "done"]
        with patch("builtins.input", side_effect=inputs):
            with patch("builtins.print"):
                faculty.modify_faculty()
        self.assertEqual(faculty.faculty_members[0].mandatory_days, {"TUE", "THU"})
...
class TestDeleteFacultyEdgeCases(unittest.TestCase):
    ...
    def test_delete_faculty_invalid_input_leaves_list_unchanged(self):
        with patch("builtins.input", return_value="not_a_number"):
            with patch("builtins.print") as mock_print:
                faculty.delete_faculty()
        printed_text = "\n".join(str(c.args[0]) for c in mock_print.call_args_list)
        self.assertIn("Invalid input. Please enter a valid number.", printed_text)
        self.assertEqual(len(faculty.faculty_members), 1)
    ...
    def test_delete_faculty_correct_member_among_multiple(self):
        ...
        self.assertEqual(len(faculty.faculty_members), 2)
        ...
        self.assertEqual(len(faculty.faculty_members), 1)
        self.assertEqual(faculty.faculty_members[0].name, "Dr. Smith")
